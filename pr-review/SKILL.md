...
PR review is the **most frequent** of the three. It happens on every change. The quality gates from the multi-contributor-synthesis-coding skill apply here, but this skill operationalizes them as specific review steps.
...
The codebase-review skill has 16 categories with tiered checks. Not all are relevant to a single PR. For delta reviews, apply selectively:
...
The codebase-review skill is the reference catalog. This PR review skill tells you which items to pull from it for a given change.
